...
v3.1.10 では、同一時刻の `full` / `mincho` / 複数行コメント群で、行数の少ない同期レイヤーだけが上側に圧縮される問題を修正しました。同期レイヤーの内部行間を論理キャンバス高と行数から計算し、同期群では共通の移動幅を使うことで時間経過によるレイヤーのほどけを抑えています。密な複数行レイヤーでは行ピッチに合わせて内部フォントサイズを抑制し、前景だけが太りすぎる問題も軽減しています。
