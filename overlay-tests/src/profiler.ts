export type OverlayDebugSample = {
  kind?: string;
  ts?: number;
  videoTimeMs?: number;
  ...
  playbackRate?: number;
  isPaused?: boolean;
  acMinVpos?: number | null;
  acMaxVpos?: number | null;
  acMinLane?: number | null;
  acMaxLane?: number | null;
  acHasScrolling?: boolean;
  error?: string;
  sampleComments?: unknown[];
};

export type OverlayProfiler = {
  clear(): void;
  getRaw(): OverlayDebugSample[];
  getStats(): {
    total: number;
    frames: number;
    events: number;
  ...
  downloadRaw(): void;
  downloadCompact(): void;
};

const overlayDebugSamples: OverlayDebugSample[] = [];

export const pushOverlaySample = (sample: OverlayDebugSample): void => {
...
export const createOverlayProfiler = (): OverlayProfiler => ({
  clear() {
  ...
  downloadCompact() {
    const compact = overlayDebugSamples.map((sample) => ({
      ...
      acMinVpos:
        sample.acMinVpos !== null && sample.acMinVpos !== undefined
          ? Math.round(sample.acMinVpos)
          : null,
      acMaxVpos:
        sample.acMaxVpos !== null && sample.acMaxVpos !== undefined
...
export const installOverlayProfiler = (): OverlayProfiler => {
...
declare global {
  interface Window {
    COOverlayProfiler: OverlayProfiler;
  }
}
