...
- [ ] 既存の `naka/ue/shita`, `small/medium/big`, `defont/gothic/mincho`, 色名/Hex の確認
  - 実装位置: 既存（`src/core/comment-commands.ts`）。
  - 受け入れ: 代表ケースで意図通りに反映。

- [ ] 拡張色名や曖昧パース（句読点除去等）の検証
  - 実装位置: 既存ロジックの回帰確認。
